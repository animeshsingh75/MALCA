...
import com.project.malca.databinding.ActivityRateUserBinding
import com.project.malca.models.FriendRating
import com.project.malca.models.Rating
                ...
                .addOnSuccessListener {
                    ...
                    FirebaseFirestore.getInstance().collection("users").document(mCurrentUid)
                        ...
                        .addOnSuccessListener {
                            ...
                            if (clicked) {
                                Log.d("Rating", "Name ${user.name}")
                                db.getReference("ratingUser").get()
                                    .addOnSuccessListener {
                                        if (it.child(friendId!!).exists()) {
                                            val rating =
                                                it.child(friendId!!).getValue(Rating::class.java)
                                            db.getReference("rating").get().addOnSuccessListener {
                                                if (it.child(friendId!!).child(mCurrentUid)
                                                        .exists()
                                                ) {
                                                    val friendRating =
                                                    ...
                                                    val timeDifference =
                                                        currentTime - friendRating!!.sentAt
                                                    if (timeDifference >= 24 * 60 * 60) {
                                                        ...
                                                        db.reference.child("rating/${friendId}/${mCurrentUid}")
                                                            .setValue(friendRMap)
                                                        ratingMap =
                                                            Rating(rating.totalPeople, newRating)
                                                    ...
                                                    db.reference.child("rating/${friendId}/${mCurrentUid}")
                                                        .setValue(friendRMap)
                                                    ratingMap =
                                                        Rating(rating.totalPeople + 1, newRating)
                                                    db.reference.child("ratingUser/${friendId}")
                                                        .setValue(ratingMap)
                                                    pushRating(avgRating)
                                                }
                                ...
                                Toast.makeText(this, "Please select a rating", Toast.LENGTH_LONG)
                                    .show()
                            }
                        }
                }
