package com.project.malca.setting
...
import com.project.malca.databinding.ActivityReviewProfileBinding
...
class ReviewProfileActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // startCrop(sentphoto)
        Picasso.get().load(sentphoto).placeholder(R.drawable.defaultavatar)
            .error(R.drawable.defaultavatar).into(binding.sentImage)
    ...
    private fun uploadImage(it: Uri) {
        ...
        uploadTask.continueWithTask(Continuation<UploadTask.TaskSnapshot, Task<Uri>> { task ->
            ...
            if (task.isSuccessful) {
                ...
                database.collection("users").document(mCurrentUid).get().addOnSuccessListener {
                    val ref =
                        storage.reference.child("profile_pics/" + mCurrentUid + "_120x120").downloadUrl.addOnSuccessListener {
                            ...
                            FirebaseFirestore.getInstance().collection("users")
                                .document(mCurrentUid).get().addOnSuccessListener {
                                    currentUser = it.toObject(User::class.java)!!
                                    currentUser = User(
                                        currentUser.name,
    ...
    //         .setInitialCropWindowPaddingRatio(0F).setAutoZoomEnabled(true).start(this)
    // }
