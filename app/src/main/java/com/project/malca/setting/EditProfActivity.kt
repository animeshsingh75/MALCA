...
import com.project.malca.databinding.ActivityEditProfBinding
...
class EditProfActivity : AppCompatActivity(), AdapterView.OnItemSelectedListener {
                    ...
                    if (changeInitial) {
                        for (datasnapshot in snapshot.children) {
                            if (datasnapshot.child(mCurrentUid).value != null && changeInitial) {
                                changeInitial = false
                                val inbox =
                                    datasnapshot.child(mCurrentUid).getValue(Inbox::class.java)
                                ...
                                Log.d("Inbox", inbox.name)
                                FirebaseDatabase.getInstance().getReference("chats").child(inbox.to)
                                    .child(inbox.from).setValue(inbox).addOnSuccessListener {
                                        finish()
                                        startActivity(
                                            Intent(
                                                this@EditProfActivity,
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == Activity.RESULT_OK && requestCode == 1000) {
            ...
            // startCrop(imageUri)
            binding.userImgView.setImageURI(imageUri)
